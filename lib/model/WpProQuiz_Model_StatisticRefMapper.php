...
// 		}
    ...
    public function fetchHistory($quizId, $page, $limit, $users = -1, $startTime = 0, $endTime = 0)
    {
        ...
        switch ($quizId) {
            case -1: //all quizs
                ...
                break;
            default:
                $whereQ = 'AND sf.quiz_id in ' . '('.$quizId.')';
                break;
        }
        switch ($users) {
            case -3: //only anonym
        ...
        $result = $this->_wpdb->get_results(
            $this->_wpdb->prepare('
				...
				FROM
					...
					LEFT JOIN ' . $this->_tableMaster . ' AS m ON(m.ID = sf.quiz_id)
					
				WHERE
					sf.is_old = 0 ' . $whereQ .$where. ' ' . $timeWhere . '
				GROUP BY
					sf.statistic_ref_id
				ORDER BY
					sf.create_time DESC
    ...
    public function countHistory($quizId, $users, $startTime = 0, $endTime = 0)
    {
        $timeWhere = '';
      ...
      //  echo 'SELECT COUNT(user_id) FROM'.'WHERE quiz_id ='. $quizId. 'AND is_old = 0'. $where. $timeWhere;
        return $this->_wpdb->get_var(
            $this->_wpdb->prepare(
                "SELECT COUNT(user_id) FROM {$this->_tableStatisticRef} WHERE is_old = 0 {$where} {$timeWhere}",
                $quizId
            )
        );
    }
