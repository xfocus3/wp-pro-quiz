...
class WpProQuiz_Controller_Front
{
    ...
    public function __construct()
    {
        ...
        add_shortcode('WpProQuiz_stat', array($this, 'shortcodeStat'));
    ...
    public function shortcodeStat($attr)
    {
        $ids = $attr[0];
        $myArray = explode(',', $ids);
        $content = '';
        ...
        if (!$this->_settings->isJsLoadInHead()) {
            $this->loadJsScripts(true, true, false);
        }

        if (is_numeric($id)) {
            ob_start();

            $this->handleShortCodeStat($ids, isset($attr['q']));
    ...
    private function handleShortCodeStat($quizId, $inQuiz = false)
    {
        $view = new WpProQuiz_View_StatisticsByQui();

        $quizMapper = new WpProQuiz_Model_QuizMapper();

        $quiz = $quizMapper->fetch(1);
