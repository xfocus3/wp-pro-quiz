...
class WpProQuiz_Controller_Statistics extends WpProQuiz_Controller_Controller
{
    ...
    public static function ajaxLoadHistory($data)
    {
        ...
        $statisticModel = $statisticRefMapper->fetchHistory($quizId, $start, $limit, get_current_user_id(), $startTime,
            $endTime);
        ...
        if (isset($data['generateNav']) && $data['generateNav']) {
            //  echo '---userID->'.$data['users'].'---quizID->'.$quizId.'$startTime, $endTime'.$startTime.$endTime;
            $count = $statisticRefMapper->countHistory($quizId, get_current_user_id(), $startTime, $endTime);
